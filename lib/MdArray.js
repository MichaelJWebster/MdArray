...
var MdArray = function(arrayArgs) {
    ...
    if (arrayArgs.shape) {
	this.dims = arrayArgs.shape;
	this.tSize = _us.reduce(this.dims, function(memo, val) {
            return memo * val;
	}, 1);
	this.strides = getStrides(this.dims);
    }
    else {
	this.dims = [arrayArgs.data.length];
	this.tSize = arrayArgs.data.length;
	this.strides = [1];
    }
...
MdArray.arange(args /* [start,] end [, by] [, shape] */) {
    'use strict';
