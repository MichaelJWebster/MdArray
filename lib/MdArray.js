...
MdArray.prototype = {
     ...
     *             rows or columns.
     *
     * @returns The sum of all elements in this MdArray, or a sum by rows or columns.
     */
    sum: function(dim) {
	var sum = 0;
	if (typeof dim === 'undefined') {
	    sum = _us.reduce(this.data, function(memo, val) {
		return memo + val;
	    }, 0);
	}
	else {
	    // dim should either "rows" or "columns"
	    assert(this.dims.length === 2,
		   "Sum across " + dim + " is only supported for 2 dimensional MdArrays.");
	    if (dim === "rows") {
		var range = _us.range(this.dims[0]);
		_us.each(range, function(val) {
		    var view = this.slice([val.toString(), ":"]);
		    sum += view.sum();
		});
	    }
	    else {
...
_us.extend (ArrayView.prototype, {
    ...
    // Note: Inefficient and messy - should redo this.
    applyOp: function(that, opFn) {
	var thisArray = [];
	var thatArray = [];
	this.foreach(function(val) {
	    thisArray.append(val);
	});
	that.foreach(function(val) {
	    thatArray.append(val);
	});
	return _.map(_.zip(thisArray, thatArray), function(x) {
