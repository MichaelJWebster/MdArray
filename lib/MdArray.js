...
MdArray.prototype = {
	...
	var view = Object.create(MdArray.prototype);
	
	view.data = this.data;
	view.strides = this.strides.slice(0);
	view.dims = this.dims.slice(0);
	view.tSize = this.tSize;
	view.dSize = this.dSize;
	return view;
    }
 ...
 */
function ArrayView(orgData, orgDims, sliceInfo) {
    this.data = data;

    var processedSlices = processSliceInfo(orgDims, sliceInfo);
    
}

function processSliceInfo(dims, sliceInfo) {
	...
	var dim = val[0];
	var sInfo = val[1].toString();
	// Either the string starts with a number or a ":"
	var matches = _us.filter(_us.rest(pat.exec(val)), function(x) {
	    return typeof x !== 'undefined';});
	assert(matches.length > 0,
	       "ArrayView: Error matching sliceInfo.");
	var sliceObject = {};
	if (/^\d+$/.test(matches[0])) {
	    // first part is a number.
	    sliceObject.start = parseInt(matches[0]);
	    matches.shift();
	    if (matches.length === 0) {
		sliceObject.end = Math.max(sliceObject.start + 1, dim);
		return sliceObject;
	    }
	    
	}
	else { /* Must be the ":" character */
	    sliceObject.start = 0;
	}
	// We either got a number or the ":" character already - in either case, if
	// we get here, we've established sliceObject.start, and the first entry
	// of the matches array should be the ":" string.
	matches.shif();
	if (matches.length == 0) {
	    sliceObject.end = dim;
	}
	else {
	    sliceObject.end = parseInt(matches[0]);
	}
	return sliceObject;
    });
